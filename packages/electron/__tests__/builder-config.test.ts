...
const WEB_VITE_CONFIG = readFileSync(resolve(ROOT_DIR, "web", "vite.config.ts"), "utf-8");
const DESKTOP_VITE_CONFIG = readFileSync(resolve(PKG_DIR, "desktop", "vite.config.ts"), "utf-8");
...
describe("electron-builder.yml", () => {
  ...
  it("root source directories for prepare-pack actually exist", () => {
    // config/ and bin/ are source dirs committed at root.
    // public/ and public-desktop/ are generated by the web and desktop Vite builds,
    // so this test validates their configured source instead of requiring prebuilt artifacts.
    for (const dir of ["config", "bin"]) {
    ...
    expect(WEB_VITE_CONFIG).toContain('outDir: "../public"');
    expect(DESKTOP_VITE_CONFIG).toContain('outDir: "../public-desktop"');
  });
